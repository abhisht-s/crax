...
def find_latest_valid_captured_response(events: list[dict]) -> CapturedResponseSelection:
    ...
    if submitted_event is None:
        return CapturedResponseSelection(
            ok=False,
            error="No verified gpt_feedback_submission_verified event was found for this run.",
            warnings=(),
            source_event=None,
            source_metadata={},
    ...
    if not matching_captures:
        return CapturedResponseSelection(
            ...
            error=(
                ...
                "gpt_feedback_submission_verified event."
            ),
            warnings=(),
            source_event=None,
            source_metadata={},
...
def select_latest_valid_extracted_codex_prompt(
    ...
    return select_valid_extracted_codex_prompt_event(
        events,
        event,
        expect_prompt_sha256=expect_prompt_sha256,
    )


def select_valid_extracted_codex_prompt_event(
    events: list[dict],
    event: dict,
    expect_prompt_sha256: str | None = None,
) -> ExtractedCodexPromptSelection:
