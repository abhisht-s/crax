...
def submit_feedback_to_chatgpt_service(
    ...
    *,
    ...
    paste_verify_timeout_seconds: float = CHATGPT_PASTE_VERIFY_TIMEOUT_SECONDS,
    ...
    submission_verify_timeout_seconds: float = CHATGPT_SUBMISSION_VERIFY_TIMEOUT_SECONDS,
    ...
    if output_path_text:
        output_path = artifact_writer(output_path_text, feedback["message"])
    ...
    copy_result = clipboard_copy_function(feedback["message"])
...
def _wait_for_pasted_marker(
    ...
    *,
    ...
    while monotonic_function() <= deadline:
        ...
        if remaining <= 0:
            break
        sleep_function(min(poll_interval_seconds, remaining))
...
def _verify_submission_marker(
    ...
    *,
    ...
    monotonic_function: Callable[[], float] = time.monotonic,
    sleep_function: Callable[[float], None] = time.sleep,
    timeout_seconds: float = CHATGPT_SUBMISSION_VERIFY_TIMEOUT_SECONDS,
