...
def evaluate_supervision_decision(diagnostics: dict | None) -> dict:
    ...
    approval_flags = _matching_flags(source_flags, APPROVAL_REQUIRED_FLAGS)
    review_flags = _matching_flags(source_flags, NEEDS_REVIEW_FLAGS)

    if approval_flags:
        high_attention = any(flag in HIGH_ATTENTION_APPROVAL_FLAGS for flag in approval_flags)
        return {
            "decision": "approval_required",
            "attention_level": "high_attention" if high_attention else "needs_review",
            "approval_required": True,
            "needs_review": True,
            "reasons": approval_flags,
            "messages": [
                "Codex appears to have violated prompt scope or touched high-risk files.",
                "Stage 4.1 records this decision only; no approval gate is enforced yet.",
            ],
            "policy_version": POLICY_VERSION,
    ...
    if review_flags:
        return {
            ...
            "messages": [
                "This run needs review but is not automatically blocked.",
                "Stage 4.1 records this decision only; execution behavior is unchanged.",
            ],
            "policy_version": POLICY_VERSION,
            "source_flags": source_flags,
        }
