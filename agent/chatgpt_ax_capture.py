...
def find_response_candidate(
    ...
    for candidate in candidates[matched_candidate.index + 1 :]:
...
def capture_response_after_feedback(
    ...
    last_response_normalized: str | None = None
    last_response_text = ""
    ...
    while time.monotonic() <= deadline:
        try:
            ...
            last_stats = stats
            match = find_response_candidate(candidates, feedback_text)
            last_match = _match_summary(match)
            if not match["ok"]:
                last_error = match["error"]
                stable_since = None
                last_response_normalized = None
                successful_polls = 0
            else:
                response_candidate: TextCandidate = match["response_candidate"]
                response_normalized = normalize_for_match(response_candidate.text)
                if response_normalized == last_response_normalized:
                    successful_polls += 1
                    ...
                    successful_polls = 1
                    stable_since = time.monotonic()
                    last_response_normalized = response_normalized
