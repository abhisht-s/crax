...
from agent.run_services import create_run_service
...
LOCAL_CONTROLLER_MAX_AUTOMATIC_STEPS_PER_WORKER = 12
LOCAL_CONTROLLER_INITIAL_CODEX_TIMEOUT_SECONDS = 300
...
def default_initial_run_executor(
    *,
    ...
    return execute_initial_direct_codex_run_service(
        ...
        else LOCAL_CONTROLLER_INITIAL_CODEX_TIMEOUT_SECONDS,
...
class LocalController:
    def __init__(
        ...
        *,
        ...
        max_automatic_steps_per_worker: int = LOCAL_CONTROLLER_MAX_AUTOMATIC_STEPS_PER_WORKER,
    ) -> None:
        ...
        self.max_automatic_steps_per_worker = max_automatic_steps_per_worker
    ...
    def _automatic_progress_loop(self, run_id: str, *, starting_burst_count: int) -> None:
        ...
        while True:
            ...
            if not read_model.routine_action_available:
                self._commit_state_from_read_model(read_model, allow_pending_snapshot=False)
                return
            if burst_count >= self.max_automatic_steps_per_worker:
                with self._lock:
                    self.session.controller_state = LOCAL_CONTROLLER_STATE_IDLE
                    self.automatic_burst_count = burst_count
                    self.automatic_burst_reason = "automatic_step_burst_limit_reached"
                    self.last_action_result_summary = {
                        "kind": "routine_progress",
                        "reason_code": "automatic_step_burst_limit_reached",
                    }
                return
