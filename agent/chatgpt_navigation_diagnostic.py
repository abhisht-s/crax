...
MAX_CHAT_SEARCH_CYCLES = 12
INITIAL_LIST_HYDRATION_TIMEOUT_SECONDS = 0.8
POST_SCROLL_HYDRATION_TIMEOUT_SECONDS = 1.4
HYDRATION_SAMPLE_INTERVAL_SECONDS = 0.2
REQUIRED_STABLE_SAMPLES = 2
NO_PROGRESS_CYCLE_THRESHOLD = 2
PROJECT_CHAT_SCROLL_MAX_ITERATIONS = MAX_CHAT_SEARCH_CYCLES
PROJECT_CHAT_SCROLL_NO_PROGRESS_LIMIT = NO_PROGRESS_CYCLE_THRESHOLD
PROJECT_CHAT_SCROLL_ACTIONS = ("AXScrollDown",)
PROJECT_CHAT_SCROLL_DELTA_Y = -360
...
def resolve_open_project_content_and_visible_chats(
    ...
    *,
    ...
    result["chat_list_container"] = _project_container_report(rows_result.get("chat_list_container") or content["chat_list_container"])
    result["more_rows_may_exist_below"] = rows_result.get("more_rows_may_exist_below", "unknown")
    result["chats_area_confirmed"] = bool(rows_result.get("chats_area_confirmed") or rows_result["status"] == "ok")
...
def _visible_project_chat_rows(
    ...
    for title_node in snapshots:
        title_text = _project_visible_row_title(title_node)
        if not title_text:
        ...
        if not _frame_is_valid(title_frame) or not _frame_intersects(content_frame, title_frame):
        ...
        if title_frame[1] < list_top - PROJECT_CHAT_ROW_GEOMETRY_TOLERANCE:
            ...
            continue
        row_node = _project_chat_row_container(title_node, snapshots_by_path, content_frame, list_top, sidebar)
        if row_node is None:
            excluded["row_container_not_found"] += 1
            continue
        row_frame = _frame_tuple(row_node.frame)
        if row_frame is None or not _frame_intersects(content_frame, row_frame):
        ...
        candidate = {
            ...
            "title": title_text,
            "preview": _project_row_preview(row_node, title_node, snapshots, project_title),
        }
    ...
    if not rows:
        empty_container = _project_empty_chat_list_container(snapshots, content, sidebar)
        return {
            "status": "visible_chat_rows_not_found",
            "error": "No visible project chat rows were found in the project content viewport.",
            "excluded_candidate_counts": dict(excluded),
            "visible_chats": [],
            "chat_list_container": empty_container or content.get("chat_list_container") or {},
            "more_rows_may_exist_below": _project_more_rows_indicator([], snapshots, empty_container or content.get("chat_list_container") or {}, content),
            "chats_area_confirmed": bool(empty_container),
        }
    ...
    chat_list_container = _project_chat_list_container(rows, snapshots_by_path, content)
    viewport_frame = _frame_tuple(chat_list_container.get("frame")) or content_frame
    visible_chats = [
    ...
    return {
        ...
        "more_rows_may_exist_below": _project_more_rows_indicator(rows, snapshots, chat_list_container, content),
...
def _project_visible_row_title(snapshot: AXElementSnapshot) -> str:
    ...
    if snapshot.role not in TEXTLIKE_ROLES and snapshot.role not in {"AXButton", "AXLink"}:
        return ""
    if snapshot.role in {"AXButton", "AXLink"} and not (snapshot.title or snapshot.description):
        return ""
    text = _project_text(snapshot)
...
def _project_visible_chat_row_report(index: int, row: dict, viewport_frame: tuple[float, float, float, float] | None) -> dict:
    ...
    display = _project_chat_row_display_title(row["title"], accessibility_text, row_node, title_node)
    return {
        "ordinal": index,
        "title": display["title"],
...
def open_chatgpt_sidebar_destination(
    *,
    ...
    if axpress_target.get("path"):
        ...
        try:
            ...
            if not post.get("inspection_available"):
                ...
                return result
            if post.get("open_outcome") == "project_opened_but_visible_chats_not_resolved":
                result.update(_autonomous_project_chat_result_fields(post))
                result.update({"outcome": "project_opened_but_visible_chats_not_resolved", "error": post.get("reason") or "Project opened, but visible chats were not resolved."})
                return result
    ...
    if post.get("inspection_available") and post.get("confirmed"):
        ...
        if project_outcome == "project_opened_but_visible_chats_not_resolved":
            result.update({"outcome": project_outcome, "error": post.get("reason") or "Project opened, but visible chats were not resolved."})
        else:
...
def open_chatgpt_project_chat(
    *,
    ...
    result["target_initially_visible"] = plan["status"] == "ready"
    ...
    return _open_ready_project_chat_plan(
        result,
        reader,
        process.pid,
        requested_project,
...
def _open_ready_project_chat_plan(
    ...
    *,
    ...
    if fresh_plan["status"] != "ready":
        ...
        return result
    if _project_chat_plan_materially_changed(plan, fresh_plan):
        result.update(_project_chat_plan_result_fields(fresh_plan))
        _apply_project_chat_count_stage_explanation(result)
...
def _bounded_project_chat_scroll_search(
    ...
    if scroll_target.get("status") != "ready":
        ...
        return _project_chat_scroll_search_result(outcome, initial_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, 0, 0, "unknown", scroll_target.get("error") or "", started_at=started)
    ...
    initial_observation = _observe_project_chat_list_hydration(
        ...
        windowserver_probe_factory,
        sleep_function,
        timeout_seconds=INITIAL_LIST_HYDRATION_TIMEOUT_SECONDS,
    ...
    if current_plan.get("status") == "ready":
        stable_found = _stable_found_project_chat_plan(
            reader,
            pid,
            project_title,
            chat_title,
        ...
        return {
            **_project_chat_scroll_search_result("ready", stable_found, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, 0, 0, "confirmed" if _project_chat_end_of_list_confirmed(stable_found) else "unknown", "", actions_performed, started_at=started),
            "plan": stable_found,
            "target_found_after_scrolling": False,
            "initial_hydration_status": initial_observation.get("classification") or "",
            "hydration_events_observed": hydration_events_observed,
            "reset_events_observed": reset_events_observed,
            "search_cycle_summaries": cycle_summaries,
        }
    if current_plan.get("status") not in {"chat_not_currently_visible", "chat_title_not_unambiguously_representable_by_accessibility"}:
        return _project_chat_scroll_search_result(current_plan["status"], current_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, 0, 0, "unknown", current_plan.get("error") or "", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "")

    for cycle in range(1, MAX_CHAT_SEARCH_CYCLES + 1):
        ...
        if current_plan["status"] == "ready":
            ...
            _accumulate_project_chat_seen_rows(stable_found, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports)
            return {
                **_project_chat_scroll_search_result("ready", stable_found, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, cycle - 1, cycle - 1, "confirmed" if _project_chat_end_of_list_confirmed(stable_found) else "unknown", "", actions_performed, started_at=started),
                "plan": stable_found,
        ...
        if current_plan["status"] not in {"chat_not_currently_visible", "chat_title_not_unambiguously_representable_by_accessibility"}:
            return _project_chat_scroll_search_result(current_plan["status"], current_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, cycle - 1, cycle - 1, "unknown", current_plan.get("error") or "", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "", hydration_events=hydration_events_observed, reset_events=reset_events_observed, cycle_summaries=cycle_summaries)
        ...
        if refreshed_target.get("status") == "ready":
            ...
            return _project_chat_scroll_search_result(outcome, current_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, cycle - 1, cycle - 1, "not_confirmed" if bottom_evidence_seen else "unknown", refreshed_target.get("error") or "Project chat list could not be reconfirmed for scrolling.", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "", hydration_events=hydration_events_observed, reset_events=reset_events_observed, cycle_summaries=cycle_summaries)

        step = _perform_project_chat_scroll_step(reader, scroll_target, scroll_service_factory)
        if step.get("status") != "ready":
            return _project_chat_scroll_search_result("chat_list_scroll_failed", current_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, cycle - 1, cycle - 1, "not_confirmed" if bottom_evidence_seen else "unknown", step.get("error") or "", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "", hydration_events=hydration_events_observed, reset_events=reset_events_observed, cycle_summaries=cycle_summaries)
        ...
        classification = observation.get("classification") or "list_unavailable"
        ...
        new_rows = int(observation.get("new_accessibility_rows") or 0)
        cycle_summaries.append(f"cycle_{cycle}: scroll_posted -> {classification} -> {new_rows}_new_rows_observed")
        if current_plan["status"] == "ready":
            stable_found = _stable_found_project_chat_plan(
                reader,
                pid,
            ...
            _accumulate_project_chat_seen_rows(stable_found, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports)
            return {
                **_project_chat_scroll_search_result("ready", stable_found, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, cycle, cycle, "confirmed" if _project_chat_end_of_list_confirmed(stable_found) else ("not_confirmed" if bottom_evidence_seen else "unknown"), "", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "", hydration_events=hydration_events_observed, reset_events=reset_events_observed, cycle_summaries=cycle_summaries),
                "plan": stable_found,
                "target_found_after_scrolling": True,
            }
        if current_plan["status"] not in {"chat_not_currently_visible", "chat_title_not_unambiguously_representable_by_accessibility"}:
            return _project_chat_scroll_search_result(current_plan["status"], current_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, cycle, cycle, "not_confirmed" if bottom_evidence_seen else "unknown", current_plan.get("error") or "", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "", hydration_events=hydration_events_observed, reset_events=reset_events_observed, cycle_summaries=cycle_summaries)
        no_meaningful_change = bool(observation.get("no_meaningful_change"))
        if bottom_evidence_seen and no_meaningful_change and _project_chat_end_of_list_confirmed(current_plan):
            return _project_chat_scroll_search_result("chat_list_end_reached_without_match", current_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, cycle, cycle, "confirmed", "End of project chat list was confirmed by bottom evidence plus an additional unchanged scroll cycle.", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "", hydration_events=hydration_events_observed, reset_events=reset_events_observed, cycle_summaries=cycle_summaries)
        if _project_chat_end_of_list_confirmed(current_plan):
            bottom_evidence_seen = True
        if no_meaningful_change and not observation.get("reset_then_changed"):
            no_progress_count += 1
        else:
            no_progress_count = 0
        if no_progress_count >= NO_PROGRESS_CYCLE_THRESHOLD:
            return _project_chat_scroll_search_result("chat_list_scroll_no_progress", current_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, cycle, cycle, "not_confirmed", "Two complete hydration windows after separate scroll pulses exposed no new rows or meaningful viewport state changes.", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "", hydration_events=hydration_events_observed, reset_events=reset_events_observed, cycle_summaries=cycle_summaries)

    return _project_chat_scroll_search_result("chat_search_budget_exhausted_without_confirmed_end", current_plan, seen_signatures, seen_titles, seen_accessibility_rows, seen_effective_viewports, scroll_target, MAX_CHAT_SEARCH_CYCLES, MAX_CHAT_SEARCH_CYCLES, "not_confirmed" if bottom_evidence_seen else "unknown", "Maximum bounded search cycles were exhausted without confirming the end of the project chat list.", actions_performed, started_at=started, initial_hydration_status=initial_observation.get("classification") or "", hydration_events=hydration_events_observed, reset_events=reset_events_observed, cycle_summaries=cycle_summaries)
...
def _observe_project_chat_list_hydration(
    ...
    *,
    ...
    max_samples = max(REQUIRED_STABLE_SAMPLES + 1, int(math.ceil(timeout_seconds / max(HYDRATION_SAMPLE_INTERVAL_SECONDS, 0.01))) + 1)
    ...
    for sample_index in range(1, max_samples):
        ...
        if state_key == (last_state.get("state_key") or ""):
            stable_samples += 1
        else:
            if state_key != (baseline.get("state_key") or ""):
                observed_changed = True
            if _project_chat_state_looks_like_reset(state, baseline, seen_state_keys, known_rows) and not reset_observed:
                reset_observed = True
            stable_samples = 1
        ...
        if plan.get("status") == "ready":
            break
        if stable_samples >= REQUIRED_STABLE_SAMPLES and sample_index + 1 >= REQUIRED_STABLE_SAMPLES:
            break
    ...
    elif final_plan.get("status") == "ready":
    ...
    return {
        ...
        "plans": plans,
        "states": states,
        "new_accessibility_rows": len(accessibility_rows_after - accessibility_rows_before),
...
def _perform_project_chat_scroll_step(reader: object, scroll_target: dict, scroll_service_factory: object) -> dict:
    if scroll_target.get("method") == "semantic_ax_scroll":
    ...
    if scroll_target.get("method") == "coregraphics_scroll":
        ...
        try:
            ...
            posted = scroller.scroll_down(point["x"], point["y"], PROJECT_CHAT_SCROLL_DELTA_Y)
        except Exception as exc:
            return {"status": "failed", "error": str(exc), "actions_performed": []}
        if not posted.get("ok"):
    ...
    return {"status": "failed", "error": "Unsupported project chat scroll method.", "actions_performed": []}
...
def _project_chat_open_plan_from_snapshots(
    ...
    if "," in chat_title:
        return {
            **base,
            "status": "chat_title_not_unambiguously_representable_by_accessibility",
            "error": "Requested chat title contains a comma and cannot be safely matched against merged accessibility row text.",
        }
    match_candidates = []
    malformed_rows = 0
    ...
    if not matches:
        status = "chat_title_not_unambiguously_representable_by_accessibility" if canonical_rows and malformed_rows == len(canonical_rows) else "chat_not_currently_visible"
        return {**base, "status": status, "error": "Requested chat title was not represented by exactly matching visible accessibility row text."}
    if len(matches) > 1:
    ...
    if not interactable.get("ok"):
        return {**base, "status": "chat_row_not_interactable", "error": interactable.get("error") or "The matching chat row is not safely interactable.", "matched_chat_row": row}
    return {
        **base,
...
def _stable_chatgpt_geometry_sample(
    ...
    *,
    ...
    latest = samples[-1]
    return {"status": "stable", "samples": len(samples), **latest}
    ...
    *,
    ...
    if kind == "project":
        resolution = resolve_open_project_content_and_visible_chats(
            requested_title,
            snapshots,
            stable.get("ax_window_frame") or _window_frame_from_metadata(window_metadata, snapshots),
            traversal_stats=stats,
            window_metadata=window_metadata,
        )
...
def _reader_perform_action(reader: object, path: str, action: str) -> None:
...
def _invoke_reader_ax_action(reader: object, path: str, action: str) -> None:
    _reader_perform_action(reader, path, action)
